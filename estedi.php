...
<?php
	...
	$stmt = oci_parse($conn, "select * from estado");
	$r = oci_execute($stmt);
	$nrows = oci_fetch_all($stmt, $results);
	if($nrows>0){
		echo "<table border><tr><th>Id de Estado</th><th>Descripcion</th></tr>";
		for ($i = 0; $i < $nrows; $i++){
			echo "<tr>\n";
...
<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">  
  Id de estado a cambiar: <input type="text" name="id" value="<?php echo $id;?>">

  <br><br>
  Descripcion Nueva: <input type="text" name="descrip" value="<?php echo $descrip;?>">
...
<?php
	...
	if ($id != ""){
		$conn = oci_connect("invetpc","1234","//localhost/xe");
		$s = oci_parse($conn, "call pro_mant_estado(:bid, :bds, 'A')");
		oci_bind_by_name($s, ":bid", $id);
